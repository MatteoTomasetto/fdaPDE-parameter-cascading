...
public:
    ArgSort(const VectorXr& value_vec) :
...
{
private:
    typedef Eigen::Matrix<int, Eigen::Dynamic, 1> IntVectorXr;
    typedef std::vector<int> IndexSet;
    ...
    static int search_greater(const VectorXr& brk, const IndexSet& ord, const Scalar& t, int start = 0)
    {
...
public:
    ...
    static void get_cauchy_point(
        const BFGSMat<Scalar, true>& bfgs, const VectorXr& x0, const VectorXr& g, const VectorXr& lb, const VectorXr& ub,
        VectorXr& xcp, VectorXr& vecc, IndexSet& newact_set, IndexSet& fv_set)
    {
