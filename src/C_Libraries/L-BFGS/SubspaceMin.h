...
private:
    ...
    // v[ind]
    static VectorXr subvec(const VectorXr& v, const IndexSet& ind)
    {
    ...
    // v[ind] = rhs
    static void subvec_assign(VectorXr& v, const IndexSet& ind, const VectorXr& rhs)
    {
        const int nsub = ind.size();
        for (int i = 0; i < nsub; i++)
            v[ind[i]] = rhs[i];
    ...
    // Check whether the VectorXr is within the bounds
    static bool in_bounds(const VectorXr& x, const VectorXr& lb, const VectorXr& ub)
    {
        const int n = x.size();
        for (int i = 0; i < n; i++)
        {
    ...
    static bool P_converged(const IndexSet& yP_set, const VectorXr& vecy, const VectorXr& vecl, const VectorXr& vecu)
    {
    ...
    static bool L_converged(const IndexSet& yL_set, const VectorXr& lambda)
    {
    ...
    // Test convergence of L set
    static bool U_converged(const IndexSet& yU_set, const VectorXr& mu)
    {
...
public:
    // bfgs:       An object that represents the BFGS approximation MatrixXr.
    // x0:         Current parameter VectorXr.
    ...
    // drt:        The output direction VectorXr, drt = xsm - x0.
    static void subspace_minimize(
        const BFGSMat<Scalar, true>& bfgs, const VectorXr& x0, const VectorXr& xcp, const VectorXr& g,
        const VectorXr& lb, const VectorXr& ub, const VectorXr& Wd, const IndexSet& newact_set, const IndexSet& fv_set, int maxit,
        VectorXr& drt)
    {
        ...
        MatrixXr WF = bfgs.Wb(fv_set);
        // Compute F'BAb = -F'WMW'AA'd
        VectorXr vecc(nfree);
        bfgs.compute_FtBAb(WF, fv_set, newact_set, Wd, drt, vecc);
        // Set the VectorXr c=F'BAb+F'g for linear term, and VectorXrs l and u for the new bounds
        VectorXr vecl(nfree), vecu(nfree);
        ...
        VectorXr yfallback = vecy;
        // Dual variables
        VectorXr lambda = VectorXr::Zero(nfree), mu = VectorXr::Zero(nfree);
        ...
        for (k = 0; k < maxit; k++)
        {
            ...
            // Indices in original drt VectorXr
            L_set.clear();
            U_set.clear();
            P_set.clear();
            // Indices in y
