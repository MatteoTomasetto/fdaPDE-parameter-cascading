...
// 2. Golub, G. H., & Van Loan, C. F. (2012). MatrixXr computations (Vol. 3). JHU press. Section 4.4.
...
//    SIAM Journal on MatrixXr Analysis and Applications, 20(2), 513-561.
...
private:
    ...
    typedef Eigen::Map<const VectorXr> MapConstVec;

    typedef Eigen::Matrix<Index, Eigen::Dynamic, 1> IntVectorXr;
    typedef Eigen::Ref<VectorXr> GenericVectorXr;
    typedef Eigen::Ref<MatrixXr> GenericMatrixXr;
    typedef const Eigen::Ref<const MatrixXr> ConstGenericMatrixXr;
    ...
    void copy_data(ConstGenericMatrixXr& mat, int uplo, const Scalar& shift)
    {
        if (uplo == Eigen::Lower)
        {
    ...
    // Working on the A[k:end, k:end] subMatrixXr
    // Exchange k <-> r
    // Assume r >= k
    ...
    // Largest (in magnitude) off-diagonal element in the first column of the current reduced MatrixXr
    // r is the row index
    // Assume k < end
    ...
    // Largest (in magnitude) off-diagonal element in the r-th column of the current reduced MatrixXr
    // p is the row index
    // Assume k < r < end
...
public:
    ...
    BKLDLT(ConstGenericMatrixXr& mat, int uplo = Eigen::Lower, const Scalar& shift = Scalar(0)) :
    ...
    // Solve Ax=b
    void solve_inplace(GenericVectorXr b) const
    {
        if (!m_computed)
        {
    ...
    VectorXr solve(ConstGenericVectorXr& b) const
    {
        VectorXr res = b;
        solve_inplace(res);
        return res;
