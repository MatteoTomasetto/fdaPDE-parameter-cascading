 ...
 *
 * Notice that the functional depends implicitly on the PDE parameters thanks to the presence of z_hat_i that is indeed
 * computed using these PDE parameters.
*/
...
{
	private: Carrier<RegressionDataElliptic, ... Extensions> * carrier_;
	...
	public: // Constructor to set the pointer to the Carrier
			...
			static const Diffusion<PDEParameterOptions::Constant> build_K(const Real& angle, const Real& intensity);
			
			// Function to build the advection vector b starting from its components
			static const Advection<PDEParameterOptions::Constant> build_b(const Real& b1, const Real& b2);

			// Functions to retrieve the value of the functional valued in the given input
			...
			// GETTERS
			inline Carrier<RegressionDataElliptic, ... Extensions>* get_carrier() const { return carrier_;};
