...
//
// An *implicit* representation of the BFGS approximation to the Hessian MatrixXr B
//
// B = theta * I - W * M * W'
// H = inv(B)
//
...
{
private:
    typedef Eigen::Ref<const VectorXr> RefConstVec;
    typedef std::vector<int> IndexSet;

    int m_m;         // Maximum number of correction VectorXrs
    Scalar m_theta;  // theta * I is the initial approximation to the Hessian MatrixXr
    MatrixXr m_s;      // History of the s VectorXrs
    MatrixXr m_y;      // History of the y VectorXrs
    VectorXr m_ys;     // History of the s'y values
    VectorXr m_alpha;  // Temporary values used in computing H * v
    int m_ncorr;     // Number of correction VectorXrs in the history, m_ncorr <= m
    int m_ptr;       // A Pointer to locate the most recent history, 1 <= m_ptr <= m
                     // Details: s and y VectorXrs are stored in cyclic order.
                     //          For example, if the current s-VectorXr is stored in m_s[, m-1],
    ...
    MatrixXr m_permMinv;             // Permutated M inverse
    BKLDLT<Scalar> m_permMsolver;  // Represents the permutated M MatrixXr
...
public:
    ...
    // n: dimension of the VectorXr to be optimized
    // m: maximum number of corrections to approximate the Hessian MatrixXr
    ...
    // Add correction VectorXrs to the BFGS MatrixXr
    inline void add_correction(const RefConstVec& s, const RefConstVec& y)
    {
        ...
        if (LBFGSB)
        {
            ...
            VectorXr Ss = m_s.leftCols(m_ncorr).transpose() * m_s.col(loc);
    ...
    inline void apply_Hv(const VectorXr& v, const Scalar& a, VectorXr& res)
    {
        res.resize(v.size());
    ...
    // Return current number of correction VectorXrs
    ...
    inline void apply_Wtv(const VectorXr& v, VectorXr& res) const
    {
    ...
    // The b-th row of the W MatrixXr
    // Preserves the ordering of Y and S columns
    // Return as a column VectorXr
    inline VectorXr Wb(int b) const
    {
        VectorXr res(2 * m_ncorr);
    ...
    // Extract rows of W
    inline MatrixXr Wb(const IndexSet& b) const
    {
        const int nb = b.size();
        const int* bptr = b.data();
        MatrixXr res(nb, 2 * m_ncorr);
    ...
    inline void apply_Mv(const VectorXr& v, VectorXr& res) const
    {
        res.resize(2 * m_ncorr);
        if (m_ncorr < 1)
            return;

        VectorXr vpadding = VectorXr::Zero(2 * m_m);
    ...
    inline bool apply_WtPv(const IndexSet& P_set, const VectorXr& v, VectorXr& res, bool test_zero = false) const
    {
    ...
    // Returns false if the result is known to be zero
    inline bool apply_PtWMv(const IndexSet& P_set, const VectorXr& v, VectorXr& res, const Scalar& scale) const
    {
    ...
    // If the P'W MatrixXr has been explicitly formed, do a direct MatrixXr multiplication
    inline bool apply_PtWMv(const MatrixXr& WP, const VectorXr& v, VectorXr& res, const Scalar& scale) const
    {
    ...
    inline void compute_FtBAb(
        const MatrixXr& WF, const IndexSet& fv_set, const IndexSet& newact_set, const VectorXr& Wd, const VectorXr& drt,
        VectorXr& res) const
    {
    ...
    //
    // v is [nP x 1]
    inline void solve_PtBP(const MatrixXr& WP, const VectorXr& v, VectorXr& res) const
    {
        const int nP = WP.rows();
        res.resize(nP);
        if (m_ncorr < 1 || nP < 1)
        ...
        // Compute the MatrixXr in the middle (only the lower triangular part is needed)
        // Remember that W = [Y, theta * S], but we do not store theta in WP
        MatrixXr mid(2 * m_ncorr, 2 * m_ncorr);
        // [0:(ncorr - 1), 0:(ncorr - 1)]
        for (int j = 0; j < m_ncorr; j++)
        {
    ...
    // P'BQv = -WP * M * WQ' * v
    // Returns false if the result is known to be zero
    inline bool apply_PtBQv(const MatrixXr& WP, const IndexSet& Q_set, const VectorXr& v, VectorXr& res, bool test_zero = false) const
    {
        const int nP = WP.rows();
    ...
    // If the Q'W MatrixXr has been explicitly formed, do a direct MatrixXr multiplication
    inline bool apply_PtBQv(const MatrixXr& WP, const MatrixXr& WQ, const VectorXr& v, VectorXr& res) const
    {
        const int nP = WP.rows();
